...
export async function POST(req: Request) {
  ...
  const { name, type, website, logo_url, brand } = await req.json()
  if (!name?.trim()) return NextResponse.json({ error: 'Name is required' }, { status: 400 })

  const { data, error } = await supabase
    .from('source')
    .insert({ name: name.trim(), type: type || null, website: website?.trim() || null, logo_url: logo_url?.trim() || null, brand: brand?.trim() || null })
    .select()
    .single()
