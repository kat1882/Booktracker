  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-3">
        {editions.map(ed => {
          const displayPrice = ed.price_override ?? ed.estimated_value
          return (
              ...
              {editing === ed.id && (
                <div className="border-t border-gray-800 p-4 bg-gray-950">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                    ...
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">eBay Est. Market Value ($)</label>
                      <input
                        type="number" step="0.01"
                    ...
                    <div>
                      <label className="text-xs text-gray-500 mb-1 block">
                        📌 Price Override ($)
                        <span className="text-gray-600 ml-1 font-normal">— overrides eBay, never auto-updated</span>
                      </label>
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 bg-gray-900 border border-amber-700 rounded px-3 py-1.5 text-sm text-white focus:outline-none focus:border-amber-500"
                        />
                        {form.price_override && (
                          <button
                            type="button"
                            onClick={() => setForm(f => ({ ...f, price_override: undefined }))}
                            className="text-xs text-gray-500 hover:text-red-400 px-2 border border-gray-700 rounded transition-colors"
