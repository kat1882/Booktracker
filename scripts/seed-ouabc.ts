/**
 * Seeds Once Upon a Book Club editions from their Shopify products API.
 * Usage: npx tsx scripts/seed-ouabc.ts
 */
...
function cleanTitle(raw: string): string {
  return raw
    .replace(/\s*[-–]\s*(once upon a book club|ouabc|book on.*|signed.*|special.*|exclusive.*|annotated.*)/gi, '')
    .replace(/\s*\((once upon[^)]*|ouabc[^)]*)\)/gi, '')
    .replace(/\s+(special edition|exclusive edition|signed edition|collector.?s edition|book only)$/gi, '')
...
async function fetchAll(): Promise<ShopifyProduct[]> {
  ...
  let page = 1
  while (true) {
    const res = await fetch(`https://onceuponabookclub.com/products.json?limit=250&page=${page}`)
...
async function run() {
  const { data: source, error } = await supabase.from('source').select('id').ilike('name', 'Once Upon a Book Club').single()
  if (error) { console.error('Source not found:', error.message); process.exit(1) }
  const sourceId = source.id
  console.log(`Once Upon a Book Club source ID: ${sourceId}\n`)

  console.log('Fetching OUABC products…')
  const all = await fetchAll()
  console.log(`Fetched ${all.length} total`)

  // Only books — OUABC uses 'Books' product_type for their special editions
  const books = all.filter(p => p.product_type === 'Books')
